...
| `admin_auth.js` | 管理者認証 / admin_users CRUD / `setupAdminSS()`（親SSの全シートをスキーマ reconcile、スキーマ変更時に手動実行） |
| `admin_branch.js` | 校舎管理 / getChildSS / getBranches / addBranch / updateBranch / `setupBranchSS()`（全校舎を一括処理: spreadsheet_id 未設定は新規作成、設定済みは reconcile。スキーマ変更時に手動実行） / `reconcileChildSchemas(cramId)`（個別校舎のスキーマ調整） |
...
| `scores_data` | 得点・順位（score_id / exam_id / student_id / subject_id / score / grade_rank / class_rank / update_at / not_taken / term_test_id / grade / year） |
