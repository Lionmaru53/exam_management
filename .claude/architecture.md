...
| `saveData.js` | 生徒向け: 得点保存（student_id / subject_id / score 等を scores_data へ）。「その他」教科は `_getOrCreateTempSubject()` で subjects_master に仮教科を自動登録し subject_id を割り当て |
...
| `subjects_master` | 教科マスター（subject_id / subject_name / genre_id / grade / default / **is_temp**）。`default=TRUE` の教科が新パターン自動生成時の初期教科。`is_temp='1'` は生徒が「その他」で入力した仮教科（`SUB{学年数字}{連番}` 形式で自動生成） |
