...
管理者（Google アカウントでログイン）
  └─→ script.google.com/macros/s/[管理者ID]/exec?page=admin  ← 直接 GAS URL（Cloudflare 経由なし）
...
| `admin_logic_branches.html` | 校舎管理 |
...
| `exam_patterns` | 学校×コース×学年×sub_course×試験区分 | 子 SS |
| `exam_schedule` | pattern_id × year → 日程 | 子 SS |
| `scores_data` | 得点・順位 | 子 SS |
| `students_master` | 生徒情報（PII） | 子 SS |
